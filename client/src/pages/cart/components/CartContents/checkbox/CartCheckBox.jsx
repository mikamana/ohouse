export default function CartCheckBox({ cart_id, checkedItemHandler, checked }) {
  const checkHandled = (e) => {
    checkedItemHandler(e.target.id, e.target.checked);
  }
  return (
    <div className='cart_contents_header_left_checkbox_wrap'>
      <input className='cart_contents_checkbox' id={cart_id} type="checkbox" checked={checked} onChange={checkHandled} />
