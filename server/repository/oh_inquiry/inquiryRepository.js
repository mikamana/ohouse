...
export async function getBeforeInquiry(pid, startIndex, endIndex) {
...
export async function getInquiry(mid, pid, startIndex, endIndex) {

  const sql = `select rno,mid,nickname,qtype,secret_check,qcontent,substring(qdate,1,10) qdate,ifnull(adate,0) adate,acontent,cnt from (select row_number() over (order by qdate desc) as rno,oi.mid,om.nickname,substring(oi.qdate,1,10) as qdate,secret_check,qtype,qcontent,adate,acontent,(select count(pid) as cnt from oh_inquiry where pid = ? group by pid) cnt from oh_inquiry oi inner join oh_member om on oi.mid = om.mid where pid = ? order by (oi.mid = ?) desc, qdate desc) as m limit ?,?`;

  return db.execute(sql, [pid, pid, mid, startIndex, endIndex])
    .then((rows) => rows[0])

}

export async function getAllInquiry(pid) {

  const sql = `select * from oh_inquiry where pid = ?`;
