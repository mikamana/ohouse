...
export async function getSortLastHouse(){
  ...
  const sql = "select hid,SUBSTRING_INDEX(oc.mid,'@',1) as mid,om.userimg,oc.house_img,oc.house_title,oc.house_content,om.mdate from oh_community oc inner join oh_member om on oc.mid = om.mid order by mdate";
  return db.execute(sql)
  .then((rows)=>rows[0])
